...
public class GuiNewChat extends Gui
{
                    ...
                    if (chatline != null)
                    {
                        ...
                        if (j1 < 200 || flag)
                        {
                            ...
                            if (l1 > 3)
                            {
                                int i2 = 0;
                                int j2 = -i1 * 9;
                                if (!Shindo.getInstance().getOptionManager().getOptionByName("Transparent Chat").isCheckToggled()) {
    ...
    {
        if (Shindo.getInstance().getOptionManager().getOptionByName("Compact Chat").isCheckToggled()) {
            if (chatComponent.getUnformattedText().equals(this.lastMessage)) {
               ...
               chatComponent.appendText(EnumChatFormatting.RED + " (" + this.sameMessageAmount + "x)");
            } else {
    ...
    {
    	IChatComponent message = chatComponent;
    	if(Shindo.getInstance().getModManager().getMod("NickHider").isToggled() && message.getFormattedText().contains(mc.thePlayer.getNameClear())) {
    		String msg = message.getFormattedText().replaceAll(mc.thePlayer.getNameClear(), Shindo.getInstance().getSettingsManager().getSettingByModAndName("NickHider", "Nickname").getText());
        	message = new ChatComponentText(msg);
    	}
        this.setChatLine(message, chatLineId, this.mc.ingameGUI.getUpdateCounter(), false);
        
        if (Shindo.getInstance().getModManager().getMod("AutoGG").isToggled() && mc.getCurrentServerData() != null && mc.getCurrentServerData().serverIP.contains("hypixel") && (chatComponent.getUnformattedText().contains("WINNER!") || chatComponent.getUnformattedText().contains("1st Killer -") || chatComponent.getUnformattedText().contains("Top Survivors"))) {
