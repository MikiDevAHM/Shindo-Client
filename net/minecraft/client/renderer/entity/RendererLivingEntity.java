...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    private FloatBuffer setRed(FloatBuffer instance, float v) {
        if (Shindo.getInstance().getModManager().getMod("Hit Color").isToggled()) {
            instance.put(Shindo.getInstance().getSettingsManager().getSettingByModAndName(
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures)
    {
        ...
        else
        {
            ...
            if (flag1)
            {
                setRed(brightnessBuffer, f);
                setGreen(brightnessBuffer, f);
                setBlue(brightnessBuffer, f);
                setAlpha(brightnessBuffer, f);
    ...
    /*
     * 
     * Mixin De Cloud Client
     * 
     */
    private Entity canRenderNameMixin(RenderManager instance) {
        if (Shindo.getInstance().getModManager().getMod("NameTag").isToggled()) {
            if (Shindo.getInstance().getSettingsManager().getSettingByModAndName(
    ...
    {
    	if(Shindo.getInstance().getModManager().getMod("NameTag").isToggled()) {
    		if(Shindo.getInstance().getSettingsManager().getSettingByModAndName("NameTag", "Disable Player NameTags").isCheckToggled()) {
    			return false;
    		}
    	}
        ...
        return Minecraft.isGuiEnabled() && entity != canRenderNameMixin(renderManager) && !entity.isInvisibleToPlayer(entityplayersp) && entity.riddenByEntity == null;
    }
