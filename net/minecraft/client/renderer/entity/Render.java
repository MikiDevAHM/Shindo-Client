...
public abstract class Render<T extends Entity> implements IEntityRenderer
{
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        ...
        int color = Shindo.getInstance().getSettingsManager().getSettingByModAndName("NameTag", "Font Color").getColor().getRGB();
        float alpha = nameTagToggled ? Shindo.getInstance().getSettingsManager().getSettingByModAndName("NameTag", "Opacity").getCurrentNumber() / 255f : 0.25f;
        float scale = nameTagToggled ? Shindo.getInstance().getSettingsManager().getSettingByModAndName("NameTag", "Size").getCurrentNumber() : 1;
        float yPos = nameTagToggled ? Shindo.getInstance().getSettingsManager().getSettingByModAndName("NameTag", "Y Position").getCurrentNumber() - 2.5f : 0;
