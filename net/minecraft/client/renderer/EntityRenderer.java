...
import me.miki.shindo.event.impl.render.RenderEvent;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                if(FreelookMod.cameraToggled) {
                	GlStateManager.translate(0.0F, 0.0F, (float) (-Helper3D.calculateCameraDistance(d0, d1, d2, d3)));
                } else {
	                GlStateManager.translate(0.0F, 0.0F, (float)(-d3));
                }
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                EntityViewRenderEvent.CameraSetup event = new EntityViewRenderEvent.CameraSetup(this, entity, block1, partialTicks, f6, f7, f8);
                event.call();
    ...
    public void setupCameraTransform(float partialTicks, int pass)
    {
        ...
        if (!Shindo.getInstance().getOptionManager().getOptionByName("Minimal View Bobbing").isCheckToggled() && mc.gameSettings.viewBobbing)
        {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            if (this.mc.gameSettings.smoothCamera)
            {
	            ...
	            this.lockPlayerLooking(mc.thePlayer ,f2, f3 * (float)i);
            }
            else
            {
                this.smoothCamYaw = 0.0F;
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        if (flag1 && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.water))
        {
            ...
            if (Shindo.getInstance().getModManager().getMod("BlockOverlay").isToggled() ) {
            	BlockOverlayMod block = new BlockOverlayMod();
            	block.drawSelectionBox(entityplayer1, this.mc.objectMouseOver, 0, partialTicks);
            } else {
