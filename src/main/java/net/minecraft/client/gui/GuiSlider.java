...
public class GuiSlider extends GuiButton
{
    ...
    protected void mouseDragged(Minecraft mc, int mouseX, int mouseY)
    {
        if (this.visible)
        {
            ...
            Helper2D.drawRoundedRectangle(xPosition  +(int)(this.sliderPosition * (float)(this.width - 8)), yPosition , 8, 20, 2,
                    Style.getColorTheme(8).getRGB(),
                    Shindo.getInstance().getOptionManager().getOptionByName("Rounded Corners").isCheckToggled() ? 0 : -1
            );

            Helper2D.drawRoundedRectangle(xPosition + 1 +(int)(this.sliderPosition * (float)(this.width - 8)) , yPosition + 1, 6, 18, 2,
