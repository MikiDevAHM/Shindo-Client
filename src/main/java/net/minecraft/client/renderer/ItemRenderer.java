...
public class ItemRenderer
{
    ...
    public float modifyTransformItem(float original, float partialTicks) {
        ...
        Setting blockSetting = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Block Animation");
        return mod.isToggled() && blockSetting.isCheckToggled() ? abstractClientPlayer.getSwingProgress(partialTicks) : original;
    }
    ...
    public void oldRod(ItemRenderer instance, EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        ...
        Mod mod = Shindo.getInstance().getModManager().getMod("Animation");
        Setting rodSetting = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Fishing Rod Animation");

        if (mod.isToggled() && rodSetting.isCheckToggled() && !mc.getRenderItem().shouldRenderItemIn3D(heldStack)) {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0)
                {
                    ...
                    {
                        case NONE:
                            this.transformFirstPersonItem(f, modifyTransformItem(0.0F, partialTicks));
                            break;
                ...
                oldRod(this, abstractclientplayer, this.itemToRender, ItemCameraTransforms.TransformType.FIRST_PERSON);
            }
    ...
    {
        Mod mod = Shindo.getInstance().getModManager().getMod("Animation");
        Setting switchSetting = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Item Switch Animation");

        if (mod.isToggled() && switchSetting.isCheckToggled()) {
