...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle("Shindo Client " + Shindo.VERSION + " for " + "Minecraft 1.8.9");
    ...
    private void sendClickBlockToController(boolean leftClick)
    {
        ...
        Setting blockAnimation = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Block Animation");
        Setting pushingSetting = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Pushing Animation");
        Setting pushingParticleSetting = Shindo.getInstance().getSettingManager().getSettingByModAndName(mod.getName(), "Pushing Particles");

        if (mod.isToggled() && blockAnimation.isCheckToggled() &&pushingSetting.isCheckToggled() && gameSettings.keyBindUseItem.isKeyDown()) {
            if (leftClickCounter <= 0 && leftClick && objectMouseOver != null && objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                if (!theWorld.isAirBlock(objectMouseOver.getBlockPos()) && thePlayer.isAllowEdit()) {
                    ...
                    if (pushingParticleSetting.isCheckToggled()) {
