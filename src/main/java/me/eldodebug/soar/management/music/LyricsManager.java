...
public class LyricsManager {
    ...
    public CompletableFuture<LyricsResponse> fetchLyrics(Track track) {
        ...
        if (apiUrl == null) {
            ShindoLogger.error("Failed to construct API URL for track: " + track.getName());
            return CompletableFuture.completedFuture(null);
        }
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    ShindoLogger.info("Failed to get lyrics, HTTP response code: " + responseCode);
                    reset();
                }

                return null;
            } catch (Exception e) {
                ShindoLogger.error("Error fetching lyrics: " + e.getMessage());
                reset();
                return null;
            }
        }, executorService);
    ...
    private String getLyricsApiUrl(String trackId) {
        ...
        try {
            ...
            ShindoLogger.error("Error formatting lyrics API URL: " + e.getMessage());
        }
    ...
    public void processLyricsRomanization(LyricsResponse lyrics) {
        ...
        RomanizationManager romanizer = Shindo.getInstance().getRomanizationManager();
        if (romanizer == null) {
            return;
        }
