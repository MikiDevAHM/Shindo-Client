	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(currentCategory.equals(CapeCategory.CUSTOM)){
			if (capeCount == 0){
				...
				nvg.drawCenteredText("(Shindo only supports capes that are PNG using Minecraft layout. You may need to restart Shindo!)", getX()+(getWidth()/2), getY() + (getHeight()/2) + 12, palette.getFontColor(ColorType.NORMAL), 7, Fonts.REGULAR);
			}
