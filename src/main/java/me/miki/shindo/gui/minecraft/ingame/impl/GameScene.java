...
public class GameScene extends GameMenuScene {
    ...
    public GameScene(GuiShindoGameMenu parent) {
        super(parent);
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)){
            ...
            this.mc.displayGuiScreen(new GuiOptions(getParent(), this.mc.gameSettings));
        }
        offset += standardPadding;
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)){
            if(this.mc.isSingleplayer() && !this.mc.getIntegratedServer().getPublic()){
                this.mc.displayGuiScreen(new GuiShareToLan(getParent()));
            } else {
        ...
        offset += standardPadding;
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)) {
            this.mc.displayGuiScreen(new GuiStats(getParent(), this.mc.thePlayer.getStatFileWriter()));
        }
        offset += standardPadding;
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)) {
            this.mc.displayGuiScreen(new GuiAchievements(getParent(), this.mc.thePlayer.getStatFileWriter()));
        }
        offset += standardPadding;
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)) {
            mc.displayGuiScreen(Shindo.getInstance().getModMenu());
        }
        offset += standardPadding;
        if (MouseUtils.isInside(mouseX, mouseY, x, y + offset, width, 22)) {
