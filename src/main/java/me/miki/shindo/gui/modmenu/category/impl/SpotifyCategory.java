...
public class SpotifyCategory extends Category implements MusicManager.TrackInfoCallback {
        ...
        musicManager.setTrackInfoCallback(this);
            ...
            CompletableFuture.runAsync(() -> {
                try {
                    ...
                    ShindoLogger.warn("Failed to sync volume slider: " + e.getMessage());
                }
            });
    }

    private void fetchUserPlaylists() {
        Shindo.getInstance().getMusicManager().getUserPlaylists()
                ...
                .exceptionally(ex -> {
                    ShindoLogger.error("Failed to fetch user playlists: " + ex.getMessage());
                    return null;
                });
    }
    ...
    private void tryOpenBrowser(String uri) {
        try {
            ...
            ShindoLogger.error(String.valueOf(e));
        }
    }
    ...
    private void drawTrackImage(NanoVGManager nvg, Track track, float offsetY) {
        ...
        String albumArtUrl = Shindo.getInstance().getMusicManager().getAlbumArtUrl(track);
    ...
    private void drawPlaylistEntry(NanoVGManager nvg, ColorPalette palette, PlaylistSimplified playlist, float offsetY) {
        ...
        String imageUrl = Shindo.getInstance().getMusicManager().getPlaylistImageUrl(playlist);
        if (imageUrl != null) {
        ...
        pendingSearch = searchDebouncer.schedule(() -> {
            if (isSearching.compareAndSet(false, true)) {
                try {
                    ...
                    Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_SEARCH_FAILED, NotificationType.ERROR);
                } finally {
    ...
    private String[] wrapText(NanoVGManager nvg, String text, float fontSize, Font font, float maxWidth) {
    ...
    private void handleTrackClick(int mouseX, int mouseY) {
            ...
            for (Track track : searchResults) {
                if (MouseUtils.isInside(mouseX, mouseY, this.getX() + 15, this.getY() + offsetY, this.getWidth() - 30, 46)) {
                    if (MouseUtils.isInside(mouseX, mouseY, this.getX() + this.getWidth() - 60, this.getY() + offsetY + 15, 16, 16)) {
                        ...
                        Shindo.getInstance().getMusicManager().play(track.getUri());
                    }
                    return;
                }
                offsetY += 56;
        ...
        if (searchPlaylistResults != null) {
            for (PlaylistSimplified playlist : searchPlaylistResults) {
                ...
                if (MouseUtils.isInside(mouseX, mouseY, this.getX() + 15, this.getY() + offsetY, this.getWidth() - 30, 46)) {
                    try {
                        Shindo.getInstance().getMusicManager().playPlaylist(playlist.getUri());
                    } catch (Exception e) {
                        ShindoLogger.error("Failed to play playlist: " + e.getMessage());
                        Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_PLAY_PLAYLIST, NotificationType.ERROR);
                    }
                    return;
                }
                offsetY += 56;
    ...
    private void addToQueue(Track track) {
        ...
        musicManager.addToQueue(track.getUri())
                .thenRun(() -> Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_ADDED_TO_QUEUE, NotificationType.SUCCESS))
                .exceptionally(ex -> {
                    Shindo.getInstance().getNotificationManager().post(TranslateText.MUSIC, TranslateText.SPOTIFY_FAILED_TO_ADD_TO_QUEUE, NotificationType.ERROR);
                    return null;
                });
    }
    ...
    private void updateVolume() {
        ...
        if (currentTime - lastVolumeChangeTime > VOLUME_CHANGE_DELAY) {
            ...
            int volume = (int) (volumeSlider.getSetting().getValueFloat() * 100);
            Shindo.getInstance().getMusicManager().setVolume(volume);
        }
    }
    ...
    private void drawConnectButton(NanoVGManager nvg, int mouseX, int mouseY) {
        ...
        AccentColor accentColor = Shindo.getInstance().getColorManager().getCurrentColor();
