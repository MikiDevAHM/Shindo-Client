...
public class ModePicker extends Patchers {
    ...
    public void renderPatchers(int mouseX, int mouseY) {
        ...
        for (int i = 0; i < patcher.getOptions().length; i++) {
            if (longestString < Shindo.getInstance().getFontHelper().size20.getStringWidth(patcher.getOptions()[i])) {
                longestString = Shindo.getInstance().getFontHelper().size20.getStringWidth(patcher.getOptions()[i]);
            }
        }
        ...
        if (isOpen()) {
            if (!animateSelect.hasFinished()) {
                GLHelper.startScissor(
                        ...
                        panel.getW(),
                        patcher.getOptions().length * 15 + 2
                );
            }
            Helper2D.drawRoundedRectangle(
                    panel.getX() + panel.getW() - 30 - longestString,
                    panel.getY() + panel.getH() + getY() - patcher.getOptions().length * 15 + animateSelect.getValueI() + 20,
                    longestString + 10, patcher.getOptions().length * 15 + 2,
                    2, Style.getColorTheme(5).getRGB(),
                    2
            );
            ...
            for (int i = 0; i < patcher.getOptions().length; i++) {
                ...
                if (hovered) {
                    Helper2D.drawRoundedRectangle(
                            panel.getX() + panel.getW() - 30 - longestString,
                            panel.getY() + panel.getH() + getY() + offset * 15 - patcher.getOptions().length * 15 + animateSelect.getValueI() + 20,
                ...
                Shindo.getInstance().getFontHelper().size20.drawString(
                        patcher.getOptions()[i],
                        panel.getX() + panel.getW() - 24 - longestString,
                        panel.getY() + panel.getH() + getY() + offset * 15 - patcher.getOptions().length * 15 + animateSelect.getValueI() + 25,
                        color
                );
