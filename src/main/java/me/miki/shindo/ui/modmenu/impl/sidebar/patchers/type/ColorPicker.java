...
public class ColorPicker extends Patchers {
    ...
    public ColorPicker(Patcher patcher, Panel panel, int y) {
    ...
    public void renderPatchers(int mouseX, int mouseY) {
        ...
        Shindo.getInstance().getFontHelper().size30.drawString(patcher.getName(), panel.getX() + 80, getYH + 6, color);
        animate.update();
        ...
        if (open) {
            ...
            sidePosHelper.pre(patcher.getSideSlider());

            if (dragSide) {
                patcher.setSideSlider(mouseY - (getYH + 25));
                float sliderHeight = 65;
                if (patcher.getSideSlider() < 0) {
                    patcher.setSideSlider(0);
                } else if (patcher.getSideSlider() > sliderHeight) {
                    patcher.setSideSlider(sliderHeight);
                }
            }

            sidePosHelper.post(patcher.getSideSlider());
            sidePosHelper.update();

            Color sideColor = ColorHelper.getColorAtPixel(getXW - 35, getYH + 28 + patcher.getSideSlider() + offset);
            if (animate.hasFinished() && getYH + 28 + patcher.getSideSlider() + offset < (panel.getY() + panel.getH() + 300))
                patcher.setSideColor(sideColor);

            float sidePosY = getYH + 25 + patcher.getSideSlider() + offset;
            ...
            Helper2D.drawHorizontalGradientRectangle(getXW - 191, getYH + 27 + offset, 146, 66, -1, patcher.getSideColor().getRGB());
            Helper2D.drawGradientRectangle(getXW - 191, getYH + 27 + offset, 146, 66, 0x00000000, 0xff000000);

            mainPosHelperX.pre(patcher.getMainSlider()[0]);
            mainPosHelperY.pre(patcher.getMainSlider()[1]);

            if (dragMain) {
                patcher.getMainSlider()[0] = mouseX - (getXW - 193);
                patcher.getMainSlider()[1] = mouseY - (getYH + 25);
                float sliderWidth = 145;
                float sliderHeight = 65;
                if (patcher.getMainSlider()[0] < 0) {
                    patcher.getMainSlider()[0] = 0;
                } else if (patcher.getMainSlider()[0] > sliderWidth) {
                    patcher.getMainSlider()[0] = sliderWidth;
                }
                if (patcher.getMainSlider()[1] < 0) {
                    patcher.getMainSlider()[1] = 0;
                } else if (patcher.getMainSlider()[1] > sliderHeight) {
                    patcher.getMainSlider()[1] = sliderHeight;
                }
            }

            mainPosHelperX.post(patcher.getMainSlider()[0]);
            mainPosHelperY.post(patcher.getMainSlider()[1]);
            mainPosHelperX.update();
            mainPosHelperY.update();

            Color mainColor = ColorHelper.getColorAtPixel(getXW - 191 + patcher.getMainSlider()[0], getYH + 28 + patcher.getMainSlider()[1] + offset);
            if (animate.hasFinished() && getYH + 28 + patcher.getMainSlider()[1] + offset < (panel.getY() + panel.getH() + 300))
                patcher.setColor(mainColor);
            float mainPosX = getXW - 193 + patcher.getMainSlider()[0];
            float mainPosY = getYH + 25 + patcher.getMainSlider()[1] + offset;
        ...
        String rgbText = "R" + patcher.getColor().getRed() + " G" + patcher.getColor().getGreen() + " B" + patcher.getColor().getBlue();
        ...
        Helper2D.drawRectangle(getXW - 38, getYH + 4, 16, 16, patcher.getColor().getRGB());
