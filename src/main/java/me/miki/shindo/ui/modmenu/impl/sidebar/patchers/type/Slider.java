...
public class Slider extends Patchers {
    ...
    public Slider(Patcher patcher, Panel panel, int y) {
        super(patcher, panel, y);
        sliderPos = patcher.getCurrentNumber() / (patcher.getMaxNumber() / sliderWidth);
    }
    ...
    public void renderPatchers(int mouseX, int mouseY) {
        ...
        Shindo.getInstance().getFontHelper().size20.drawString(
                String.valueOf(MathHelper.round(patcher.getCurrentNumber(), 1)),
                panel.getX() + panel.getW() - 175 -
                        Shindo.getInstance().getFontHelper().size20.getStringWidth(String.valueOf(MathHelper.round(patcher.getCurrentNumber(), 1))),
        ...
        if (drag) {
            ...
            if (diff == 0) {
                patcher.setCurrentNumber(patcher.getMinNumber());
            } else {
                patcher.setCurrentNumber(sliderPos * (patcher.getMaxNumber() / sliderWidth));
            }
        }
