package me.miki.shindo.api.hooks;
    ...
    public static boolean validateResourcePackUrl(NetHandlerPlayClient client, S48PacketResourcePackSend packet) {
        try {
            ...
            if (isLevelProtocol && (url.contains("..") || !url.endsWith("/resources.zip"))) {
                logger.logError(LoggerWarningEnum.ProjectWarning, "Malicious server tried to access {}" + url);
                if (Minecraft.getMinecraft().thePlayer != null) {
                   Shindo.getInstance().getMessageHelper().showMessage("Shindo", "&c&lThe current server has attempted to be malicious but we have stopped them.");
                }
